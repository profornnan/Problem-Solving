...
struct SegmentTree {
	...
	Node init(int cur, int start, int end, const vector<int>& nums) {
		...
		if (start == end) return now = { nums[start - 1], nums[start - 1], nums[start - 1], nums[start - 1] };

		int mid = start + (end - start) / 2;
		Node leftChild = init(cur * 2, start, mid, nums);
		Node rightChild = init(cur * 2 + 1, mid + 1, end, nums);

		now.leftSum = max(leftChild.leftSum, leftChild.totalSum + rightChild.leftSum);
		now.rightSum = max(leftChild.rightSum + rightChild.totalSum, rightChild.rightSum);
		now.totalSum = leftChild.totalSum + rightChild.totalSum;
		now.maxSum = max({ leftChild.maxSum, rightChild.maxSum, leftChild.rightSum + rightChild.leftSum });

		return now;
	}

	Node query(int cur, int start, int end, const int left, const int right) {
		if (right < start || end < left) return { -INF, -INF, 0, -INF };
		...
		Node res;
		res.leftSum = max(leftChild.leftSum, leftChild.totalSum + rightChild.leftSum);
		res.rightSum = max(leftChild.rightSum + rightChild.totalSum, rightChild.rightSum);
		res.totalSum = leftChild.totalSum + rightChild.totalSum;
		res.maxSum = max({ leftChild.maxSum, rightChild.maxSum, leftChild.rightSum + rightChild.leftSum });
