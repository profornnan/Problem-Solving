...
struct Planet {
	int pos, id;
	bool operator<(const Planet& next) {
		return pos < next.pos;
	}
};
...
int main(void) {
	...
	for (int i = 0; i < N - 1; i++) {
		edge.push_back({ planetX[i].id, planetX[i + 1].id, planetX[i + 1].pos - planetX[i].pos });
		edge.push_back({ planetY[i].id, planetY[i + 1].id, planetY[i + 1].pos - planetY[i].pos });
		edge.push_back({ planetZ[i].id, planetZ[i + 1].id, planetZ[i + 1].pos - planetZ[i].pos });
	}
